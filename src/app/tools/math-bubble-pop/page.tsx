...
/**
 * Kids Math — Bubble Pop. A bubble-popping game that teaches missing-number
 * equations: kids tap the bubble holding the number that completes the
 * equation, with ten-frame tips when they get stuck, plus a printable
 * worksheet. Themed with the site's design tokens; all CSS is scoped under
 * `.math-tool` and mapped to the site's shadcn variables so light/dark mode
 * follows the rest of the site.
 */
export default function MathBubblePopPage() {
  const mountRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    document.title = 'Bubble Pop: Missing Number Game for Kids - Great Seattle Life Hacks';
    ...
    styleEl.setAttribute('data-math-tool', 'bubble-pop');
